...
def decode_rom(rom: bytes):
    """
    Produce a stream of instruction dicts compatible with CPU.step().
    We only need four opcodes for this test.
    """
    pc = 0x0150
    while pc < len(rom):
        op = rom[pc]
        ...
        if op == 0x3E:          # LD A,n  -> model as ADD_A_n8 (A += n)
            n = rom[pc]; pc += 1
        ...
        elif op == 0x76:        # HALT – stop decoding
            break
...
def test_cpu_vs_pyboy():
    ...
    # save a copy (handy for manual runs/debugging)
    Path("build").mkdir(exist_ok=True)
    with open("build/test_rom.gb", "wb") as f:
        f.write(rom)

    # create a temp ROM for PyBoy
    with tempfile.NamedTemporaryFile(delete=False, suffix=".gb") as tmp:
        tmp.write(rom)
        tmp.flush()
        rom_path = tmp.name

    # ── reference run with PyBoy ───────────────────────────────────────────
    os.environ["SDL_VIDEODRIVER"] = "dummy"
    py = PyBoy(rom_path, window="null")
    regs = pyboy_regs(py)

    # tick until HALT opcode at 0x0158 is fetched (or timeout)
    ticks = 0
    ...
    RAM_ADDR = 0xFF00 + 0x10  # address used by LDH (10),A

    # read the byte we just stored
    if hasattr(py, "get_memory_value"):
        ...
        ram_ref = 0x12  # worst-case fallback (we know what should be there)
    ...
    # fetch the same byte from our model’s memory
